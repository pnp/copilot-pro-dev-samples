...
import { UserInfo, withAuth } from "./middleware/auth";
...
export async function meHandler(
  ...
  try {
    // For demo purposes, create a default consultant object based on userInfo
    // In a real app, you would look up the user in your database

    const me = await ConsultantApiService.getApiConsultantByEmail(userInfo?.email);
    ...
    switch (req.method) {
      ...
      case "POST": {
        // Use the same me object we created above
        switch (command) {
          case "chargetime": {
            try {
            ...
            if (body) {
              ...
              const result = await ConsultantApiService.chargeTimeToProject(projectName, userInfo.id, hours);
