...
Once configured, you can ask Copilot Chat for a random number, for example, `Give me 3 random numbers`. It should prompt you to use the `get_random_number` tool on the `CustomersMCPServer` MCP server and show you the results.

## Security Toggle

You can enable or disable authentication and authorization together using this setting in `appsettings.json`:
...
- `true`: Authentication and authorization are enabled.
- `false`: Authentication and authorization are disabled (the MCP endpoint is open).
...
1. When using VS Code, connecting to `https://localhost:5247` fails.
  ...
  * Connecting with `http://localhost:6200` succeeds.
